...
const UpcomingTasks = ({ searchQuery }) => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    return taskData.filter(item => {
      ...
      const matchSearch = searchQuery ? item.title.toLowerCase().includes(searchQuery.toLowerCase()) : true

      const taskTime = item.date ? new Date(item.date).getTime() : 0
      const isupcoming = taskTime > next24Hours
